...
def simple_dot_from_src(file_name):
    arch_cmd = 'Identify the signal-slot connections between the objects from the code below. Write the graph in DOT language where the nodes are objects and edges are the connections.\n'
...
def dot_from_src(file_name):
    arch_cmd = 'Identify the signal-slot connections between the QT objects from the code below. Write the graph in DOT language where the nodes are objects and edges are the connections. The node label should include specific object name. The edge label should include the connection method used ("connect" or "disconnect").\n'
    code = load_query(file_name)
    # code = truncate_to_token_limit(code, 3000)
    print('Generating DOT code ...')
    arch_desc = get_answer(arch_cmd + code)
